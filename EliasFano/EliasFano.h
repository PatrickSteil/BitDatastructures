...
public:
  EliasFanoEncoding(const std::vector<unsigned long long> &numbers)
    ...
    upperSize = std::floor(std::log2(numElements));

    // std::cout << numElements << "\t" << upperSize << "\t" << (64 - upperSize) << std::endl;

    std::vector<bool> upperBitsVEC((numElements << 1) + 1, false);
    ...
    // upperBits.printVector();
  }

  // Predecessor Method
  unsigned long long pred(unsigned long long x) const {
    // see https://arxiv.org/pdf/2003.11835.pdf
    if ((unsigned long long) (*this)[0] >= x) return std::numeric_limits<unsigned long long>::infinity();
    if ((unsigned long long) (*this)[numElements-1] <= x) return (*this)[numElements-1];

    unsigned long long xHIGH = (x >> (64 - upperSize));
    unsigned long long xLOW = (x & lowerMASK());
    unsigned long long lower(0);
    if (xHIGH > 0) lower = upperBits.select(0, xHIGH - 1) - xHIGH + 1;
    ...
    if (lower > 0 && getLowerAsInt(lower) > xLOW) return (*this)[lower-1];
    return (*this)[upper];
  }
  ...
  unsigned long long operator[](unsigned long long index) const {
    ...
    unsigned long long result = static_cast<unsigned long long>(((unsigned long long) upperBits.select(1, index) - index) << (64 - upperSize));

    // std::cout << upperBits.select(1, index) << "\t" << (upperBits.select(1, index)-index) << "\t" << std::bitset<64>(result) << std::endl;

    return static_cast<unsigned long long>(result | static_cast<unsigned long long>(getLowerAsInt(index)));
  }
  ...
  inline unsigned long long lowerMASK() const { return (unsigned long long)(~0) >> upperSize; }
