...
public:
  EliasFano(std::vector<uint64_t> &numbers) : numElements(numbers.size()) {
    uint64_t maxElement = numbers[numElements - 1];
    // offset is needed to reduce the universe to the max element occuring in
    // numbers
    ...
    // false);
    // I fill these vectors at first, and then pass these to the corresponding
    // BitVectors
    std::vector<bool> upperBitsVEC(numElements * 3 + 1, false);
    std::vector<bool> lowerBitsVEC(numElements * (64 - offset - upperSize) + 1,
                                   false);
    ...
    for (uint64_t element : numbers) {
      ...
      for (uint64_t j = 0; j < (64 - offset - upperSize); ++j) {
        lowerBitsVEC[i * (64 - offset - upperSize) + j] =
            ((LSB >> ((64 - offset - upperSize) - 1 - j)) & 1);
      }
      ++i;
    }
  ...
  uint64_t operator[](uint64_t index) const {
    ...
    uint64_t result =
        static_cast<uint64_t>(((uint64_t)upperBits.select(1, index) - index)
                              << (64 - offset - upperSize));

    return static_cast<uint64_t>(result |
                                 static_cast<uint64_t>(getLowerAsInt(index)));
  }

  inline uint64_t getLowerAsInt(uint64_t index) const {
    assert(valid_index(index));
    uint64_t lower = 0;
    // TODO: maybe splice the bits faster together instead of bit per bit
  ...
  inline uint64_t lowerMASK() const { return (uint64_t)(~0) >> (upperSize); }

  inline bool valid_index(uint64_t index) const { return index < numElements; }
