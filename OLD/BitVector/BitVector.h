...
public:
    BitVector(size_t numBits = 64)
        : numBits(numBits)
        , blockSize(std::floor(std::log2(numBits) / 2.0))
    ...
    inline uint64_t rankFalse(int i) const
    {
        assertmsg(0 <= i && (size_t)i < numBits, "index out of bounds!");
        ...
        uint64_t val = superBlockZeros[superBlockPos(i)] + blockZeros[blockPos(i)];
    ...
    inline uint64_t rankTrue(int i) const { return i - rankFalse(i); }

private:
    template <bool PARITY = true>
    inline uint64_t select(int count) const
    {
